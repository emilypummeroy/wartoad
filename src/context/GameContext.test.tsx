...
import { getPondStateAt, INITIAL_POND, LEAF_COUNT_PER_ROW, ROW_COUNT, setPondStateAt } from '../state/pond';
import { CardClass, UnitClass } from '../types/card';
import { Phase, Player, Subphase } from '../types/gameflow';
import type { Position } from '../types/position';
import { commitActivate, GameContext, type GameDispatch, type GameState } from './GameContext';
import { activationOf, createStateWith, gameflowOf } from './GameContext.test-utils';
...
describe(GameContext, () => {
  describe(commitActivate, () => {
    describe.for<[string, Position, start: Position | undefined, Player, Subphase, Phase]>([
    ...
    ])('when precondition failed: %s | target: %s | start: %s | flow: %s %s %s', ([_, target, start, ...flow]) => {
    ...
    const it_should_end_activation_and_return_to_idle_state = (target: Position, old: GameState) => {
    ...
    const it_should_not_affect_anything_else = (target: Position, start: Position, old: GameState) => {
    ...
    describe.for<[target: Position, Player, targetUnits: number]>([
      ...
      ([target, player, targetCount]) => {
        let i = 0;
        const unit = createUnit({ cardClass: Froglet, owner: player, key: (i += 1) });
        ...
        const old = createStateWith({
          ...
          pond: setPondStateAt(INITIAL_POND, target, { units: [...targetUnits, unit] }),
        });
    ...
    describe.for<[target: Position, start: Position, Player, targetUnits: number, startUnits: number]>([
      ...
      ([target, start, player, targetCount, startCount]) => {
        ...
        const old = createStateWith({
          ...
          ...gameflowOf(player, Activating),
          pond: setPondStateAt(setPondStateAt(INITIAL_POND, start, { units: [...startUnits, unit] }), target, {
            units: targetUnits,
          }),
        });

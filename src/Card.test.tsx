  ...
  describe.for<Player>([Player.North, Player.South])(
    ...
    player => {
      it('should be a simple card back if it is not a leaf', () => {
        ...
        expect(
          within(card).getByRole('img', { name: 'Card back' }),
        ).toBeVisible();
      });
      ...
      it(`should be shown as a ${player} controlled leaf if it is a leaf`, () => {
        ...
        expect(card).toBeVisible();
        expect(
          within(card).getByRole('img', { name: `${player} controlled leaf` }),
        ).toBeVisible();
      });
    },
  );
...
describe(Froglet, () => {
  describe.for<Player>([Player.North, Player.South])(
    ...
    player => {
      it('should have Froglet cost if not on a leaf', () => {
        ...
        expect(card).toBeVisible();
        expect(
          within(card).getByRole('group', { name: /Cost/ }),
        ).toHaveTextContent(/0/);
      });
      ...
      it(`should be shown as a ${player} unit if it is on a leaf`, () => {
        ...
        expect(card).toBeVisible();
        expect(
          within(card).getByRole('img', { name: `${player} unit` }),
        ).toBeVisible();
      });
    },
  );
  ...
  describe.for<[Player, isOnLeaf?: boolean, string?]>([
    ...
    'basic cases | player: %s | isOnLeaf: %s | id: "%s"',
    ([player, isOnLeaf, id]) => {
      it('should have Froglet stats', () => {
        ...
        expect(
          within(card).getByRole('group', { name: 'Stats' }),
        ).toHaveTextContent('0 Power 0 Range 1 Speed 1 Life');
      });
    },
  );
});
...
describe(LilyPad, () => {
  describe.for<Player>([Player.North, Player.South])(
    ...
    player => {
      ...
      it(`should be shown as ${player} controlled if it is a leaf but not home`, () => {
        ...
        expect(card).toBeVisible();
        expect(
          within(card).getByRole('img', { name: `${player} controlled` }),
        ).toBeVisible();
      });
      ...
      it(`should be shown as ${player} Home if it is a home leaf`, () => {
        ...
        expect(card).toBeVisible();
        expect(
          within(card).getByRole('img', { name: `${player} Home` }),
        ).toBeVisible();
      });
    },
  );
  ...
  describe.for<[Player, isLeaf?: boolean, isHome?: boolean, string?]>([
    ...
    'basic cases | player: %s | isLeaf: %s | id: "%s"',
    ([player, isLeaf, isHome, id]) => {
      it('should have Lily Pad stats', () => {
        ...
        expect(
          within(card).getByRole('group', { name: 'Gives' }),
        ).toHaveTextContent(/0/);
      });
    },
  );
